...
--*/
{
	...
	gTimeStampArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(UINT64) * MAX_RECORD_COUNT, 'tSrT');
	gProcessIdArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(HANDLE) * MAX_RECORD_COUNT, 'pIrP');
	gFileSizeArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(LARGE_INTEGER) * MAX_RECORD_COUNT, 'fSzF');
	gIsRenameArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(UINT8) * MAX_RECORD_COUNT, 'iRnI');
	gIsDeleteArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(UINT8) * MAX_RECORD_COUNT, 'iDlI');
	gFileNameSuffix = ExAllocatePool2(
		POOL_FLAG_NON_PAGED,
		sizeof(*gFileNameSuffix) * MAX_RECORD_COUNT,
		'nSfF'
	);
	gIsReadArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(UINT8) * MAX_RECORD_COUNT, 'iRad');
	gIsWriteArray = ExAllocatePool2(POOL_FLAG_NON_PAGED,
		sizeof(UINT8) * MAX_RECORD_COUNT, 'iWri');

	if (!gTimeStampArray || !gProcessIdArray || !gFileSizeArray ||
		!gIsRenameArray || !gIsDeleteArray || !gFileNameSuffix) {
		if (gTimeStampArray) ExFreePoolWithTag(gTimeStampArray, 'tSrT');
		if (gProcessIdArray) ExFreePoolWithTag(gProcessIdArray, 'pIrP');
		if (gFileSizeArray)  ExFreePoolWithTag(gFileSizeArray, 'fSzF');
		if (gIsRenameArray)  ExFreePoolWithTag(gIsRenameArray, 'iRnI');
		if (gIsDeleteArray)  ExFreePoolWithTag(gIsDeleteArray, 'iDlI');
		if (gFileNameSuffix) ExFreePoolWithTag(gFileNameSuffix, 'nSfF');
		if (gIsReadArray)    ExFreePoolWithTag(gIsReadArray, 'iRad');
		if (gIsWriteArray)   ExFreePoolWithTag(gIsWriteArray, 'iWri');
...
--*/
{
	...
	RtlInitUnicodeString(&uniName, L"\\DosDevices\\C:\\Windows\\MyMinifilterLog.bin");
	...
	ULONG count = (ULONG)(gRecordIndex + 1);

	if (count > MAX_RECORD_COUNT) {
		count = MAX_RECORD_COUNT;
	}

	DbgPrint("records: %lu", count);

	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb, &count, sizeof(ULONG), NULL, NULL);
	...
	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gFileSizeArray, sizeof(LARGE_INTEGER) * count, NULL, NULL);
	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gIsRenameArray, sizeof(UINT8) * count, NULL, NULL);
	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gIsDeleteArray, sizeof(UINT8) * count, NULL, NULL);

	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gFileNameSuffix, sizeof(WCHAR) * FILE_SUFFIX_BUFFER * count,
		NULL, NULL);

	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gIsReadArray, sizeof(UINT8) * count, NULL, NULL);
	ZwWriteFile(fileHandle, NULL, NULL, NULL, &iosb,
		gIsWriteArray, sizeof(UINT8) * count, NULL, NULL);

	ZwFlushBuffersFile(fileHandle, &iosb);

	ZwClose(fileHandle);

	ExFreePoolWithTag(gTimeStampArray, 'tSrT');
	ExFreePoolWithTag(gProcessIdArray, 'pIrP');
	ExFreePoolWithTag(gFileSizeArray, 'fSzF');
	ExFreePoolWithTag(gIsRenameArray, 'iRnI');
	ExFreePoolWithTag(gIsDeleteArray, 'iDlI');
	ExFreePoolWithTag(gIsReadArray, 'iRad');
	ExFreePoolWithTag(gIsWriteArray, 'iWri');
...
inline void ProcessIRP(PFLT_CALLBACK_DATA Data, PCFLT_RELATED_OBJECTS FltObjects) {
	UNREFERENCED_PARAMETER(Data);
	UNREFERENCED_PARAMETER(FltObjects);
	...
	////UINT64 time = timeLarge.QuadPart;
	...
	//{
	//	FsRtlGetFileSize(iopb->TargetFileObject, &file_size);
	//}

	//UCHAR OP;
	//OP = Data->Iopb->MajorFunction;
	...
	//	{
	//		PFILE_DISPOSITION_INFORMATION dispositionInfo = (PFILE_DISPOSITION_INFORMATION)iopb->Parameters.SetFileInformation.InfoBuffer;
	...
	//	// Do something
	//	UNICODE_STRING usName = nameInfo->Name;
	//	USHORT totalChars = usName.Length / sizeof(WCHAR);
	//	USHORT suffixStart = (totalChars > FILE_SUFFIX_LEN)
	//		? (totalChars - FILE_SUFFIX_LEN)
	//		: 0;
	//	USHORT copyCount = totalChars - suffixStart;  // <= FILE_SUFFIX_LEN

	//	RtlZeroMemory(gFileNameSuffix[slot],
	//		sizeof(WCHAR) * FILE_SUFFIX_BUFFER);

	//	RtlCopyMemory(
	//		gFileNameSuffix[slot],
	//		&usName.Buffer[suffixStart],
	//		sizeof(WCHAR) * copyCount
	//	);
	...
	LONG slot = InterlockedIncrement(&gRecordIndex);
	if (slot < MAX_RECORD_COUNT) {
		// record time
		KeQuerySystemTimePrecise(gTimeStampArray + slot);

		// record
		gProcessIdArray[slot] = PsGetCurrentProcessId();

		// record filesize
		if ((iopb->MajorFunction == IRP_MJ_READ || iopb->MajorFunction == IRP_MJ_WRITE) &&
			iopb->TargetFileObject != NULL)
		{
			FsRtlGetFileSize(iopb->TargetFileObject, gFileSizeArray + slot);
		}
		else {
			gFileSizeArray[slot].QuadPart = 0;
		}

		gIsReadArray[slot] = iopb->MajorFunction == IRP_MJ_READ;
		gIsWriteArray[slot] = iopb->MajorFunction == IRP_MJ_WRITE;
		...
		// record whether is_delete or is_rename
		switch (iopb->MajorFunction) {
		case IRP_MJ_SET_INFORMATION:
			switch (iopb->Parameters.SetFileInformation.FileInformationClass) {
			case FileRenameInformation:
			case FileRenameInformationEx:
				gIsRenameArray[slot] = 1;
				break;
			case FileDispositionInformation:
			case FileDispositionInformationEx:
			{
